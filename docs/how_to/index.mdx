...
在这里，您将找到“我该如何……？”类型问题的答案。这些指南是*目标导向*和*具体*的；它们旨在帮助您完成特定任务。有关概念性解释，请参见[概念指南](/docs/concepts/)。有关端到端的操作步骤，请参见[教程](/docs/tutorials)。有关每个类和函数的全面描述，请参见[API 参考](https://api.python.langchain.com/en/latest/)。

## 安装

- [如何安装 LangChain 包](/docs/how_to/installation/)
- [如何在不同的 Pydantic 版本中使用 LangChain](/docs/how_to/pydantic_compatibility)
...
[LangChain 表达式语言](/docs/concepts/#langchain-expression-language-lcel) 是创建任意自定义链的一种方式。它基于 [Runnable](https://api.python.langchain.com/en/latest/runnables/langchain_core.runnables.base.Runnable.html) 协议构建。

[**LCEL 速查表**](/docs/how_to/lcel_cheatsheet/): 快速了解如何使用主要的 LCEL 原语。

[**迁移指南**](/docs/versions/migrating_chains): 将遗留链抽象迁移到 LCEL。

- [如何：链式运行 runnable](/docs/how_to/sequence)
- [如何：流式运行 runnable](/docs/how_to/streaming)
- [如何：并行调用 runnable](/docs/how_to/parallel/)
- [如何：为 runnable 添加默认调用参数](/docs/how_to/binding/)
- [如何：将任何函数转换为 runnable](/docs/how_to/functions)
- [如何：将输入从一个链步骤传递到下一个](/docs/how_to/passthrough)
- [如何：在运行时配置 runnable 行为](/docs/how_to/configure)
- [如何：为链添加消息历史（记忆）](/docs/how_to/message_history)
- [如何：在子链之间路由](/docs/how_to/routing)
- [如何：创建动态（自构建）链](/docs/how_to/dynamic_chain/)
- [如何：检查 runnable](/docs/how_to/inspect)
- [如何：为 runnable 添加后备选项](/docs/how_to/fallbacks)
- [如何：将运行时秘密传递给 runnable](/docs/how_to/runnable_runtime_secrets)
...
[提示模板](/docs/concepts/#prompt-templates) 负责将用户输入格式化为可以传递给语言模型的格式。
...
[示例选择器](/docs/concepts/#example-selectors) 负责选择正确的少量示例以传递给提示。
...
[聊天模型](/docs/concepts/#chat-models) 是一种新型的语言模型，能够接收消息并输出消息。
...
- [如何：跨提供者跟踪响应元数据](/docs/how_to/response_metadata)
...
- [如何：绑定模型特定格式的工具](/docs/how_to/tools_model_specific)
...
[消息](/docs/concepts/#messages) 是聊天模型的输入和输出。它们包含一些 `content` 和一个 `role`，用于描述消息的来源。

- [如何：修剪消息](/docs/how_to/trim_messages/)
- [如何：过滤消息](/docs/how_to/filter_messages/)
- [如何：合并连续的相同类型消息](/docs/how_to/merge_message_runs/)

### LLMs

LangChain所称的[LLMs](/docs/concepts/#llms)是较早的语言模型形式，它们接收一个字符串并输出一个字符串。
...
[输出解析器](/docs/concepts/#output-parsers) 负责将 LLM 的输出解析为更结构化的格式。
...
[文本拆分器](/docs/concepts/#text-splitters) 将文档拆分为可用于检索的块。

- [如何：递归拆分文本](/docs/how_to/recursive_text_splitter)
- [如何：按 HTML 头部拆分](/docs/how_to/HTML_header_metadata_splitter)
- [如何：按 HTML 区段拆分](/docs/how_to/HTML_section_aware_splitter)
- [如何：按字符拆分](/docs/how_to/character_text_splitter)
- [如何：拆分代码](/docs/how_to/code_splitter)
- [如何：按头部拆分 Markdown](/docs/how_to/markdown_header_metadata_splitter)
- [如何：递归拆分 JSON](/docs/how_to/recursive_json_splitter)
- [如何：将文本拆分为语义块](/docs/how_to/semantic-chunker)
- [如何：按标记拆分](/docs/how_to/split_by_token)

### 嵌入模型

[嵌入模型](/docs/concepts/#embedding-models) 将一段文本转换为其数值表示。
...
[向量存储](/docs/concepts/#vector-stores) 是能够高效存储和检索嵌入的数据库。
...
- [如何：向检索器结果添加相似性评分](/docs/how_to/add_scores_retriever)
- [如何：组合多个检索器的结果](/docs/how_to/ensemble_retriever)
- [如何：重新排序检索结果以减轻“中间丢失”效应](/docs/how_to/long_context_reorder)
...
LangChain [工具](/docs/concepts/#tools) 包含工具的描述（传递给语言模型）以及调用的函数实现。有关预构建工具的列表，请参阅 [此处](/docs/integrations/tools/)。
...
有关代理的详细操作指南，请查看 [LangGraph](https://langchain-ai.github.io/langgraph/) 文档。
...
[回调](/docs/concepts/#callbacks) 允许您在 LLM 应用程序执行的各个阶段进行挂钩。

- [如何：在运行时传入回调](/docs/how_to/callbacks_runtime)
- [如何：将回调附加到模块](/docs/how_to/callbacks_attach)
- [如何：将回调传入模块构造函数](/docs/how_to/callbacks_constructor)
...
检索增强生成（RAG）是一种将大型语言模型（LLMs）与外部数据源连接的方法。
有关 RAG 的高级教程，请查看 [本指南](/docs/tutorials/rag/)。

- [如何：添加聊天记录](/docs/how_to/qa_chat_history_how_to/)
- [如何：流式传输](/docs/how_to/qa_streaming/)
- [如何：返回来源](/docs/how_to/qa_sources/)
- [如何：返回引用](/docs/how_to/qa_citations/)
- [如何：按用户检索](/docs/how_to/qa_per_user/)

### 提取

提取是指使用LLMs从非结构化文本中提取结构化信息。
...
- [如何：管理大量聊天记录](/docs/how_to/trim_messages/)

### 查询分析

查询分析是使用 LLM 生成要发送给检索器的查询的任务。有关查询分析的高级教程，请查看 [此指南](/docs/tutorials/query_analysis/)。
...
- [如何：构造过滤器](/docs/how_to/query_constructing_filters)
- [如何：处理高基数分类变量](/docs/how_to/query_high_cardinality)

### SQL + CSV 的问答

您可以使用 LLMs 对表格数据进行问答。
...
LangGraph 文档目前托管在一个单独的网站上。您可以在此处查看 [LangGraph 使用指南](https://langchain-ai.github.io/langgraph/how-tos/)。

## [LangSmith](https://docs.smith.langchain.com/)

LangSmith 允许您紧密跟踪、监控和评估您的 LLM 应用程序。
它与 LangChain 和 LangGraph 无缝集成，您可以在构建时使用它检查和调试链和代理的各个步骤。

LangSmith 文档托管在一个单独的网站上。
您可以在这里浏览 [LangSmith 使用指南](https://docs.smith.langchain.com/how_to_guides/)，但我们将突出一些与 LangChain 特别相关的部分：
...
要了解更多信息，请查看[LangSmith评估操作指南](https://docs.smith.langchain.com/how_to_guides#evaluation).
...
您可以在 [LangSmith 文档的此部分](https://docs.smith.langchain.com/how_to_guides/tracing) 查看与跟踪相关的一般操作指南。