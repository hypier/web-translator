...
从 [`Open City Data`](/docs/integrations/document_loaders/open_city_data) 创建一个 GeoPandas 数据框作为示例输入。
...
将 GeoPandas 数据框作为 `Document` 加载以进行后续处理（嵌入、聊天等）。

`geometry` 将作为默认的 `page_content` 列，所有其他列将放置在 `metadata` 中。
