...
此页面介绍如何在 LangChain 中使用 `Dataherald API`。
...
```
- 前往 dataherald 并在 [这里](https://www.dataherald.com) 注册
- 创建一个应用并获取你的 `API KEY`
...
存在一个 DataheraldAPIWrapper 实用工具，它封装了这个 API。要导入此实用工具：

```python
from langchain_community.utilities.dataherald import DataheraldAPIWrapper
```

有关此封装的更详细说明，请参见 [此笔记本](/docs/integrations/tools/dataherald)。
