...
|Text Embedding|将字符串嵌入向量|[embed](https://docs.cohere.com/reference/embed)|`from langchain_cohere import CohereEmbeddings`|[cohere.ipynb](/docs/integrations/text_embedding/cohere)|
...
Cohere [聊天模型](/docs/integrations/chat/cohere) 的使用方法
...
messages = [
    HumanMessage(
        content="magic_function(2)的值是多少？"
    )
]
...
print(res.content)
```
通过将必要的工具绑定到 llm，可以进行与 Cohere LLM 的工具调用，如上所示。另一种方法是支持使用 ReAct 代理进行多跳工具调用，如下所示。
...
使用 Cohere [RAG 检索器](/docs/integrations/retrievers/cohere)
...
Cohere [文本嵌入模型](/docs/integrations/text_embedding/cohere) 的使用方法

### Reranker

Cohere [Reranker](/docs/integrations/retrievers/cohere-reranker) 的使用方法